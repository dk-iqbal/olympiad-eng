...
*/
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: string, index: number) => {
    ...
    if (fieldName && fieldName === 'requirement') {
      const newFields = [...requirementFields as any]
  ...
  */
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (newErrors) {
      ...
      try {
        ...
        router.push('/model-test/list')
        toast.success(`Model Test ${id ? 'Updated' : 'Created'} Successfully`)
      } catch (error) {
        console.log(error)
  ...
  useEffect(() => {
    if (selectedItem) {
      ...
      setFaqFields(parseInfo?.faqQuestions || [])
    }
  }, [selectedItem])

  return (
    <Grid container justifyContent='center' alignContent='center'>
      <Form onSubmit={onSubmit} heading={id ? 'Update Model Test Form' : 'Create Model Test Form'}>
        <>
        ...
        <Grid container justifyContent='center' gap={2} alignItems='center'>
          ...
          <SaveButton text={id ? 'Update Model Test' : 'New Model Test'} />
        </Grid>
      </Form>
    </Grid>
  )
