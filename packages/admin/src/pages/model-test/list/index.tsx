...
*/
  const onCourseDeactivation = async (item: any) => {
    try {
      ...
      toast.success('Model Test Deactivated Successfully')
    ...
    <BaseLayout
      ...
      isActionButton={
        <Button
          name='Add New'
          endIcon={<Add />}
          onClick={() => router.push('/model-test/form')}
    ...
    >
      ...
      <Paper sx={{ mt: { xs: 1, sm: 2 } }}>
                ...
                >
                  {activeCourses?.map((course: any, index: number) => {
                    const actions = [
                      ...
                      { icon: <Lock color='primary' />, name: 'Approved' },
                      {
                    ...
                    return (
                      <ListItemComponent
                        actions={actions}
                        index={index}
                        navigationPage= '/model-test/details'
                        item={modifyObject}
        ...
        <Dialog
          ...
          loading={loading}
          headertext='Model Test Deactivation'
