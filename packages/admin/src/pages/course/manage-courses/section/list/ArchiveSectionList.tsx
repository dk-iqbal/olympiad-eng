...
function ArchiveSectionList() {
  ...
  const fetchActiveSectionByStatus = async () => {
    try {
      setLoading(true)
      const response = await getSectionsByStatus(USER_STATUS.INACTIVE)
          ...
          <List sx={{ width: '100%', mt: 1 }}>
            {sections?.length > 0 ? (
              sections?.map((item: any, index: number) => {
                ...
                const actions = [
                  { icon: <LockOpen color='info' />, name: 'Section Activation' }
                ]
...
export default ArchiveSectionList
