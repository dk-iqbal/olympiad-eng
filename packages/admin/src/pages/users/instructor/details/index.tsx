...
const UserDetails = () => {
  ...
  const parseInfo = typeof selectedItem?.info === 'string' ? JSON.parse(selectedItem?.info) : selectedItem?.info
  const doctorData = [
    { label: 'Email', value: selectedItem?.email },
    { label: 'Status', value: selectedItem?.status ? 'Active' : 'Deactive' },
    { label: 'Degree', value: parseInfo?.degree },
    { label: 'Social Link', value: parseInfo?.socialLink }
  ].filter(f => f.value)
  ...
  */
      ...
      case 0: {
        router.push(`/reset-password`, {
          query: {
            ...
            navigation: '/users/instructor/list'
          }
        })
        break
      }
  ...
  const editNavigate = `/users/instructor/form`
