...
function ActiveInstructorList() {
  ...
  */
  ...
  const fetchAllActiveUsers = async () => {
    setLoading(true)
    try {
      const response = await fetchUsers(USER_ROLE.INSTRUCTOR, USER_STATUS.ACTIVE)
            ...
            instructorUsers?.map((item: any, index: number) => {
              ...
              return (
                <CommonListDetails
                  ...
                  key={item.username}
                  item={updateItem}
                  navigatePath='/users/instructor/details'
                  index={index}
...
export default ActiveInstructorList
